...
*Note:* Despite the name of the repository, the core script in this repository has command line switches to behave as link:https://www.monitoring-plugins.org/[monitoring plugin^] for other monitoring systems (e.g. Icinga, LibreNMS, Nagios) or as sender for Zabbix trapper.
See the xref:active_check#[notes on active checks] in case you are _not_ using Checkmk.

== Compatibility

Currently, Checkmk version 2.3.0 to 2.5.0 and PatchMon 1.4.x up to 2.1.x are supported.
Support for Checkmk 2.5.0 is complete and using the new link:https://docs.checkmk.com/plugin-api/2.5.0/cmk.server_side_programs/index.html#cmk.server_side_programs.v1_unstable.Storage[storage API^] instead of temporary files.
// Do not expect support for PatchMon 1.3.7 to appear.

== Installation

*If you are familiar with PatchMon, but not yet with Checkmk, read the link:./README_Checkmk4Patchmon_users.asciidoc[README_Checkmk4Patchmon_users].*
...
For Checkmk Raw/Community, starting with PatchMon 1.4.2, Iby has added the possibility to download a CSV list of the hosts present in PatchMon for easy import in Checkmk.
